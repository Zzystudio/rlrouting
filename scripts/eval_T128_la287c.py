"""LA287c argmax NAM 全 19 条 T=128 复核（近胜局判定，度量规范要求）。"""
import json
import os
import sys
import time
...
OUT = 'benchmark/routed/audit_T128_la287c_nam.json'
...
for fname in circuits:
    ...
    d = json.load(open(f'benchmark/routed/la287c_argmax/{name}.json'))
    phys = qasm2_loads(d['routed_qasm'].replace(
        'include "qelib1.inc";', 'include "qelib1.inc";\n' + SWAP_DEF))
    run_one('LA287c', fname, phys, d['num_swaps'], not d.get('completed', True))

by = {}
for r in results:
    by.setdefault(r['circuit'], {})[r['method']] = r['fid']
print('\n=== T=128 NAM 全 19 条：SABRE vs LA287c ===')
w = l = t = 0
for c in sorted(by):
    s, m = by[c]['SABRE'], by[c]['LA287c']
    mark = 'LA287c' if m > s + 1e-9 else ('SABRE' if s > m + 1e-9 else '≈')
    if m > s + 1e-9: w += 1
    elif s > m + 1e-9: l += 1
    else: t += 1
    print(f'{c:<24s} SABRE={s:.4f}  LA287c={m:.4f}  -> {mark}')
print(f'\n战绩: LA287c {w} 胜 / {l} 负 / {t} 平')
sm = sum(by[c]['SABRE'] for c in by)/len(by)
mm = sum(by[c]['LA287c'] for c in by)/len(by)
print(f'fid_mean: SABRE={sm:.4f}  LA287c={mm:.4f}  ({(mm-sm)/sm*100:+.1f}%)')
